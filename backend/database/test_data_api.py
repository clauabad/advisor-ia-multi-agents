...
def get_current_region():
    ...
    return session.region_name or os.getenv('DEFAULT_AWS_REGION', 'ca-central-1')
...
def get_cluster_details(region):
        ...
        try:
            ...
            if response['DBClusters']:
                ...
                if not cluster.get('HttpEndpointEnabled', False):
                    ...
                    print("Run: aws rds modify-db-cluster --db-cluster-identifier advisor-aurora-cluster --enable-http-endpoint --apply-immediately")
                    return None, None
            else:
    ...
    try:
        # Get cluster ARN
        response = rds_client.describe_db_clusters(
            DBClusterIdentifier='advisor-aurora-cluster'
        )
        
        if not response['DBClusters']:
            print("Aurora cluster 'advisor-aurora-cluster' not found")
            return None, None
        ...
        # Find the most recently created aurora secret for advisor
        secrets = secrets_client.list_secrets()
        aurora_secrets = []
        
        for secret in secrets['SecretList']:
            if 'aurora' in secret['Name'].lower() and 'advisor' in secret['Name'].lower():
...
def test_data_api(cluster_arn, secret_arn, region):
    ...
    try:
        ...
        if error_code == 'BadRequestException':
            ...
            print(f"Database 'advisor' might not exist or credentials are incorrect")
            ...
            try:
                ...
                print(f"Connection successful (but 'advisor' database may not exist)")
                return True
            except:
                pass
        else:
    ...
    print("\n2️Checking for existing tables...")
    try:
        response = client.execute_statement(
            resourceArn=cluster_arn,
            secretArn=secret_arn,
    ...
    try:
        response = client.execute_statement(
            ...
            secretArn=secret_arn,
            database='advisor',
            sql="SELECT pg_database_size('advisor') as size_bytes"
        )
