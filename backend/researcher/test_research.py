...
def get_service_url():
    """Get the App Runner service URL from AWS (JSON output)."""
    try:
        region = "us-east-1"

        # Get list of services (JSON)
        ...
        service = next(
            (s for s in services["ServiceSummaryList"]
             if s["ServiceName"] == "advisor-researcher"),
            None
        )

        if not service:
            print("App Runner service 'advisor-researcher' not found.")
            sys.exit(1)
        ...
        return service_details["Service"]["ServiceUrl"]
...
def main():
    parser = argparse.ArgumentParser(
        description="Test the Advisor Researcher service",
