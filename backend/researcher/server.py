"""
Advisor Researcher Service - Investment Advice Agent
"""
...
app = FastAPI(title="Advisor Researcher Service")
...
async def run_research_agent(topic: str = None) -> str:
    ...
    with trace("Researcher"):
        async with create_playwright_mcp_server(timeout_seconds=60) as playwright_mcp:
            agent = Agent(
                name="Advisor Investment Researcher",
                instructions=get_agent_instructions(),
                model=model,
                tools=[ingest_financial_document, tavily_search],
...
async def health():
    ...
    return {
        "service": "Advisor Researcher",
        "status": "healthy",
        "advisor_api_configured": bool(os.getenv("ADVISOR_API_ENDPOINT") and os.getenv("ADVISOR_API_KEY")),
        "timestamp": datetime.now(UTC).isoformat(),
...
async def test_bedrock():
    ...
    try:
        ...
        os.environ["AWS_REGION_NAME"] = "ca-central-1"
        os.environ["AWS_REGION"] = "ca-central-1"
        os.environ["AWS_DEFAULT_REGION"] = "ca-central-1"
