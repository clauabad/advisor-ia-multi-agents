...
resource "aws_sqs_queue" "analysis_jobs_dlq" {
  name ="advisor-analysis-jobs-dlq"
...
resource "aws_iam_role" "lambda_agents_role" {
  name = "advisor-lambda-agents-role"
...
resource "aws_iam_role_policy" "lambda_agents_policy" {
  name = "advisor-lambda-agents-policy"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # Lambda invocation for orchestrator to call other agents
      {
        ...
        Resource = "arn:aws:lambda:${var.aws_region}:${data.aws_caller_identity.current.account_id}:function:advisor-*"
      },
...
resource "aws_s3_bucket" "lambda_packages" {
  bucket = "advisor-lambda-packages-${data.aws_caller_identity.current.account_id}"
...
resource "aws_lambda_function" "reporter" {
  function_name = "advisor-reporter"
  role          = aws_iam_role.lambda_agents_role.arn
...
resource "aws_lambda_function" "retirement" {
  function_name = "advisor-retirement"
  role          = aws_iam_role.lambda_agents_role.arn
...
resource "aws_cloudwatch_log_group" "agent_logs" {
  ...
  name              = "/aws/lambda/advisor-${each.key}"
  retention_in_days = 7
