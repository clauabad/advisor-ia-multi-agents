...
resource "aws_iam_role" "app_runner_instance_role" {
  name = "advisor-app-runner-instance-role"
...
resource "aws_iam_role_policy" "app_runner_instance_bedrock_access" {
  name = "advisor-app-runner-instance-bedrock-policy"
...
resource "aws_apprunner_service" "researcher" {
  service_name = "advisor-researcher"
    ...
    image_repository {
      ...
      image_configuration {
        ...
        runtime_environment_variables = {
          ...
          ADVISOR_API_ENDPOINT = var.advisor_api_endpoint
          ADVISOR_API_KEY      = var.advisor_api_key
...
resource "aws_iam_role" "eventbridge_role" {
  ...
  name  = "advisor-eventbridge-scheduler-role"
...
resource "aws_lambda_function" "scheduler_lambda" {
  count         = var.scheduler_enabled ? 1 : 0
  function_name = "advisor-researcher-scheduler"
...
resource "aws_iam_role" "lambda_scheduler_role" {
  count = var.scheduler_enabled ? 1 : 0
  name  = "advisor-scheduler-lambda-role"
...
resource "aws_scheduler_schedule" "research_schedule" {
  count = var.scheduler_enabled ? 1 : 0
  name  = "advisor-research-schedule"
